...
from EvalFunc import multi_acc, generate_confusion_matrix
...
def trainPASNet(train_x, train_y, eval_x, eval_y, pathway_mask, \
			...
			Learning_Rate, L2_Lambda, nEpochs, Dropout_Rates, optimizer = "Adam", save_dir = ".", save_model = "model.pt", save_pic = "result.png"):
	...
	for epoch in range(nEpochs):
		...
		loss = cross_entropy_for_imbalance(pred, train_y) ###calculate loss
		acc = multi_acc(pred, train_y) ###calculate accuracy
		print(f'Epoch {epoch+0:03}: | train_loss: {loss:.5f} | train_acc: {acc:.3f}')
		...
		for name, param in copy_state_dict.items():
			...
			for S in S_set:
				...
				loss_tmp = cross_entropy_for_imbalance(y_tmp, train_y)
		...
		if epoch == nEpochs - 1:
			...
			train_loss = cross_entropy_for_imbalance(train_pred, train_y).view(1,)

			net.eval()
			eval_pred = net(eval_x)
			eval_loss = cross_entropy_for_imbalance(eval_pred, eval_y).view(1,)

			### Save model  and confusion matrix
			if(save_dir is not None and save_model is not None):
				torch.save(net.state_dict(), save_dir + "/" + save_model)
			if(save_dir is not None and save_pic is not None):
				generate_confusion_matrix(eval_pred, eval_y, save_dir, save_pic)
